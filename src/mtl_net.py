...
class MultiTaskNet(nn.Module):
    ...
    def init_mlp_layers(self, layer_sizes):
        ...
        mlp_layers =  [ nn.Linear(layer_sizes[0], layer_sizes[1]),
                        nn.ReLU()]
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        for user_id, item_id in zip(user_ids, item_ids): 
            user_emb = self.U(user_id)
            item_emb = self.Q(item_id)
            bias_emb = self.B(item_id)
        # Regression head
            rij_hat = torch.concat((user_emb, item_emb, user_emb*item_emb),dim = -1)
            for layer in self.mlp_layers:
                rij_hat = layer(rij_hat)
            score = torch.cat((score, rij_hat), dim = 0)

        # Matrix Factorization Head  
            pij = torch.dot(user_emb.t(), item_emb) + bias_emb 
            predictions = torch.cat((predictions, pij), dim = 0)
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        for user_id, item_id in zip(user_ids, item_ids):
            user_emb = self.U_reg(user_id)
            item_emb = self.Q_reg(item_id)
            bias_emb = self.B(item_id)
        # Regression head            
            rij_hat = torch.concat((user_emb, item_emb, user_emb*item_emb),dim = -1)
            for layer in self.mlp_layers:
            ...
            pij = torch.dot(self.U_fact(user_id).t(), self.Q_fact(item_id)) + bias_emb
            predictions = torch.cat((predictions, pij), dim = 0) 
        return predictions, score